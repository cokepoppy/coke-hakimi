 ...
 */
/* Local override: optionally skip panel ID reads on write-only MIPI links. */
...
#define ST7701_HW_RESET_PULSE_MS 10
#define ST7701_HW_RESET_RECOVERY_MS 120
...
esp_err_t esp_lcd_new_panel_st7701_mipi(const esp_lcd_panel_io_handle_t io, const esp_lcd_panel_dev_config_t *panel_dev_config, esp_lcd_panel_handle_t *ret_panel)
{
    ...
    st7701->flags.skip_id_read = vendor_config->flags.skip_mipi_id_read;
...
static esp_err_t panel_st7701_del(esp_lcd_panel_t *panel)
{
    ...
    ESP_RETURN_ON_ERROR(st7701->del(panel), TAG, "del st7701 panel failed");
...
static esp_err_t panel_st7701_reset(esp_lcd_panel_t *panel)
{
    ...
    if (st7701->reset_gpio_num >= 0) {
        ...
        vTaskDelay(pdMS_TO_TICKS(ST7701_HW_RESET_PULSE_MS));
        gpio_set_level(st7701->reset_gpio_num, !st7701->flags.reset_level);
        // ST7701S v1.2 section 7.5.5 forbids Sleep Out during the first
        // 120ms after RESX release. Waiting here also makes cold boot match
        // an already-warm panel instead of racing its internal NVM load.
        vTaskDelay(pdMS_TO_TICKS(ST7701_HW_RESET_RECOVERY_MS));
...
static esp_err_t panel_st7701_init(esp_lcd_panel_t *panel)
{
    ...
    if (st7701->flags.skip_id_read) {
        ESP_LOGI(TAG, "Skip LCD ID read on write-only MIPI link");
    } else {
        uint8_t ID[3];
